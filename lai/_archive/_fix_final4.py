...
new_fn = [
    ...
    "  const rw = (l,v) => (v && String(v) !== 'undefined' && String(v).trim() !== '') ?\n",
    ...
    "    rw('Mature height',ht)+rw('Growth rate',esc(p.growth_rate_label||''))+\n",
    "    rw('Climate',esc(p.native_koppen||''))+rw('Drought tolerance',esc(p.drought_tolerance||''))+\n",
    ...
    "    rw('Root depth',esc(p.root_depth||''))+rw('Moisture',esc(p.moisture_use||''))+\n",
...
print(f"Replaced lines {start}-{end}. Done.")
